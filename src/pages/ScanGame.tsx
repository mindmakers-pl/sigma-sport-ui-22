...
const ScanGame = () => {
  ...
  const handleClickTarget = (isTarget: boolean) => {
    if (!isTarget || gameState !== "playing" || startTime === null) return;
          ...
          <div className="w-full max-w-4xl animate-fade-in">
            <div className="grid grid-cols-10 gap-2 bg-slate-800 p-4 rounded-lg">
              {grid.map((item, index) => (
                <button
                  key={index}
                  onClick={() => handleClickTarget(item.isTarget)}
          ...
          <Card className="max-w-md w-full border-slate-700 bg-slate-800 animate-scale-in">
            <CardContent className="pt-6 text-center space-y-6">
              ...
              <div className="py-8">
                ...
                <p className="text-5xl font-bold text-primary">{finalResult} ms</p>
              </div>
              <div className="space-y-3">
                <Button 
                  ...
                  className="w-full"
                  onClick={() => {
                    console.log("Zapisz wynik:", finalResult);
                  }}
                >
                  Zapisz Wynik
                </Button>
